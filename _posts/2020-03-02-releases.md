...
### 21 Feb 2022
```
- Ability to collaborate on every lead
- Ability to know if the lead has Retired, MovedOut. So that you can filter and act on them differently.
- 
```
### 27 Dec 2021
```
- handle processing beyond 1 year
- handle multiple languages identified in the response
- Name email mismatch identification 
- help within app using google search
```
### 22 Dec 2021
```
- reserve % from the total leads limit for EmailCC
- improved model with 90% accuracy and "Moved Out" classification
```
### 6 Dec 2021
```
- make emailcc default as false
- Route guard added for edit assistant,  nurture, notification changes to avoid loosing the edited value
- ability to search the custom message for a string in the message body. 
- move default and options from Edit Assistant to Preference
- ability to search in notification and preference
```
### 24 Nov 2021
```
- Add Lead when assistant is Cc'ed in email` is enabled by default
- org admin should be able to CC any assistant in the org
- ability to have conversation based followups. Same lead email id can have two different conversations being followed up. Conversations are identified with email Subject.
- Fixes on UI
```
## 16 Nov 2021
```
- Assistant will automatically activate the leads with New status when the capacity becomes available. Preference for this to put off, if required.
- Preference to set the Lead name in the Assistant Copied scenario. 
- no open tracking for test email
```
## 15 Nov 2021
```
- Out of box Quotation followup messages in library.
- Recent Changes Notification
```
## 28 Oct 2021
```
- Show Active leads count in the dashboard
- Ability to click on the todays followup link and get the complete list of those leads
- Assistant to move out the lead NSD to a date where it is likely not to hit the DayLimit with in next 15 days.
- Ability to filter leads which has Substate "Plan Cancelled or Expired" and "Per Day Limit hit many times"
- Admin UI. Way to add/buy additional leads
- Admin UI. allow setting the expiry date to null
- Fix for reminder failure when notification is set to off
```
## 26 Oct 2021
```
- Plan changes to be based on active nurturing leads. USer can add as many leads as wanted. 
- do not send the received email as attachment to avoid spam and phishing emails
- Include Subject to Email body while detecting status
- Handle the excel upload if there is no header (first row)
- possible fix for Stripe payment while subscribing
- sequence page example date is shown as day 1, day 3 etc..
- on re upload of excel, do not update or restart leads which are already Deactivated(invalid email) or Unsubscribed
```
## 19 Oct 2021
```
- ability to have additional leads in the plan
- handle Non English Names in emails
- responded lead list should not show OOO in lead list 
- fix for CRM restart lead. setting the followup to 1.
- performance fix for responses tab
```
## 07 Oct 2021
```
- New sign-up user name should be capitalized for Tina Assistant
- ability to understand lead responses in non-english and mark the lead state correctly. supported_languages = ['ar', 'hi', 'ko', 'zh-TW', 'ja', 'zh', 'de', 'pt', 'en', 'it', 'fr','es']
- fix for failure when the lead name in the email is non-english e.g. `Éric Lem <eric@somecompany.com>`
- fix for failure from Hubspot and other CRMs while update for JSON parsing
- UI : sequence page to support pagination for sequence
- All actions in the More menu for single lead too 
- Handle the failure for empty first line in the uploaded excel
```
## 05 Oct 2021
```
- Send invalid links in nurture table and custom messages in weekly summary email
- Change user to owner from primary pages other than org user
- Fix for bullet items in the custom message. Ability to do bulleted items and sub items
```
## 28 Sept 2021
```
- fix for the Complete Followup count in the lead list filter
- First assistant creation, website field pre-populated with login email domain
- User changed to owner in leads details view
- ability to remove specific email from all notifications
```
## 24 Sept 2021
```
- enhanced model for response classification
- uploaded lead names(first and last) should be proper cased in case of all upper or lower case are uploaded from the excel file
```
## 23 Sept 2021
```
- lead clicked link should be marked Warm
- fix for Reply Followup default as per assistant preference
- fix for Clone of Sequence not working
- fix for Schedule not selectable while add lead.
- fix for Cancel not working on Schedule edit
- fix for lead asking for "company profile" is not getting the brochure
```
## 22 Sept 2021
```
- Lead email cannot be some Assistant
- Add Lead button always available in the bar
- Quill tool bar to have left and right shift actions
- Auto added leads to have tag as AutoAdded
```
## 16 Sept 2021
```
- fix for having the scroll bar for custom message 
- fix for edit lead not to set leads at risk
- fix for distributed the NSD evenly to handle the load of high of leads assigned from Hubspot.
```
## 15 Sept 2021
```
- Allow to edit assistant signature from mobile
- How To and FAQ added in navbar
- Ability to Change Assistant and Change User
- Ability to Test the Custom Message by sending an email
- fix editing the VA with existing VA email should fail
- Fix for save button on sequence description change
```
## 07 Sept 2021
```
- schedule is now available on mobile
- add multiple lead page FAQ button added and some aesthetic changes
- fix for failure of Manually Deactivated leads
- fix for failure of Unsubscribed lead
- Columns of export lead list should be consistent with Import columns
```
## 03 Sept 2021
```
- Reply Followup to not change the schedule. Assistant level preference for ReplyFollowup schedule.
- fix for schedule email getting shown as Broadcast in timeline
- ooo, linkClicked counts in Dashboard
- do not consider ooo as part of responded count in dashboard count
- fix for accepting emails send to assistant in Bcc
```
## 25 Aug 2021
```
- Pagination for Assistant page
- Fix for handling the received response from some other email with same domain (lead forwarded to some other person in the company)
```
## 20 Aug 2021
```
- Edit multiple leads
- Add Lead from Assistant page, schedule selection fix
```
## 19 Aug 2021
```
- fix for custom messages list to be searchable and with pagination when trying ti use custom message in reply action
- fix for search of custom message in the list
- fix to use https API links for CRM integrations
- fix in received email processor when trying to find lead by email
- Fix for engaged list not matching with count
- in Reply drawer, followup lead changed to view lead if the lead already exists
- Confirm edit custom message modal fixed for mobile view
- Forgot password beside error message while signing in
```
## 13 Aug 2021
```
- fix to handle the open email handler failure for deactivated org
- fix to handle the timeout for hubspot processing when a lot of contacts are assigned together from Hubspot
- fix to show "Delivery Failed" sub status for those Deactivated lead and not consider them in the used count.
- Dashboard graphs to show monthly and quarterly correctly
- add lead button on each assistant
- in schedule, while selecting custom messages, user should just be able to scroll till the end of the list (infinite scroll)
```
## 11 Aug 2021
```
- fix for Schedule Clone and Rename button
- fix for Save button getting hidden behind the onboarding steps
- Assistant leads number can be clicked to go to leads list
- fix for create custom message and list going blank
- not showing settings link if assistant is not created
- Placeholder text in confirm password while signup.
- login page, if the password validation fails then 'Want to create a new account? link to take user to signup page
- custom messages in mobile too 
- fix for jobadder failures
- fix for hubspot failures
- fix for not able to search using text when filtered by Responded state
- fix for putting notes in CRM when the NSD is moved out because of bounce rate
- accept workemailid in the uploaded excel. Intelli import
- improvisation for debounce failures. to reduce failures
```
## 06 Aug 2021
```
- fix for lead not getting saved for email in OOO content. If one of the email fails for validation
- fix for not saving duplicate response
```
## 04 Aug 2021
```
- Subject line score in broadcast message
- Response tab more options radio buttons simplification
```
## 03 Aug 2021
```
- Onboard tour - sequence changed, dynamic days values in email tabs
- Pipedrive integration. Assistant user not required any more in Pipedrive
- Nurture Suggest. If API times out then assistant sends an email to user to click the link and see the nurture rows
- Message Library now available in mobile view.
- Do not send weekly and monthly summary emails to users who are not CONFIRMED
- Two step upload process. Upload first and validate then someone else can verify and add leads
- TVL made primary and Worked on count in bracket on dashboard
- Assistant preference page now showing errors at right place
```
## 30 Jul 2021
```
Message Library Mobile view
```
## 29 Jul 2021
```
- ability to only upload the excel file and then someone else can verify and add leads
- fix for schedule not showing custom messages beyond 200
- Suggest nurturing step in onboarding
- fix for ellipses in email and schedule name
- fix for look and feel of each message in the messages tab
- fix for messages tab crashing on mobile
- get the assistant count from a summarised table to fix the performance reasons
- get the default dashboard (Quarter) counters from a daily calculated counters table for performance reasons
```
## 20 Jul 2021
```
...
## 16 Jul 2021
```
- fix for single lead showing someone elses timeline
- add assistant make website mandatory
- Company input moved to top and auto focus on for first assistant creation
- invite user notification simplified and instant
- remind me, user can select time as well along with date
- fix for failure when auto adding leads from OOO emails
- Stop sending schedule emails to leads for users with cancelled or expired plans/subscription
```
## 14 Jul 2021
```
- Onboarding Steps. If user has added leads from outside of Add Lead (old customers) then do not have Add Lead Step
- Clicked Link filter on dashboard
- Lead List button text change
- Open Position Title fields in UI at multiple places for recruitment industry
- Allow decimal values for after_days in schedule
- Hide rename button when clicked on clone button in Schedule
```
## 09 Jul 2021
```
- Ability to set off repeat warm notification for same lead 
- Show the lead in expanded mode when there is only one lead, so that timeline view is seen
- Ease of use for users who call leads, so that they can filter, click the number, add tags and then filter by tags the next day
- Job Title as part of preference for Recruitment only companies
- Ability to click and test the company url while adding the first assistant
```
## 07 Jul 2021
```
- Ability to provide Job Title in excel upload for Recruitment customers
- Deactivated users should be removed from all notifications
```
## 06 Jul 2021
```
- link to org notifications added in assistant notifications and vice versa
- Editing link in nurture, usability fix
- Schedule button for both bottom & right section
- Fix for bulk lead upload failure
- Release the use of response classifier model to all users
- Fix for handling the transient bounce from SendGrid
- Removed word mostly from Zoho & HubSpot notifications
```
## 02 Jul 2021
```
- Fix for the progress bar for processed leads
- Fix for the user notification failure
- Fix for user bounce handler failures
- Title in default signature is Business Development Executive
- Org Notification tab on mobile
- Stop summary email after 15 days of trial end
```
## 30 June 2021
```
- Assistant to have Ability to send using SendGrid as well as AWS SES
- After bulk upload wait for 4 days to check if the bounce rate. If not in limit then notify support.
- Fix for non UTF8 in Assistant copied
- Do not notify or take action if bounce is transient
- User stage watcher. On trialEnd, do not mark the lead processed.
- Changed template for WhatsApp.
- AI Sensed data to be opened by default in Mobile
- Done button in Mobile while typing email on mobile. So that the Reply Followup and other options can be seen before sending.
```
## 28 June 2021
```
- Invite User. Ask for email before first and last name.
- Edit Name icon always visible.
- Ease of adding some simple Notes like "Not Interested", "Call later", "Voice mail", etc. 
```
## 27 June 2021
```
- drag and drop for nurture row
- ability to edit phoneno too along with name
- Show date year for the message
- fix for date time now show properly on low resolution screens
```
## 23 June 2021
```
...
- Ability to create custom messages from the sequences tab itself.
