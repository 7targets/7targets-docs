...
Copy the schedules from 7Targets to Zoho by clicking on `Get Sequences`. If you intend to provide the Sequence while the lead/deal is assigned to the Assistant in Zoho, then the schedules from 7Targets has to be copied in Zoho's field `7Targets Sequence`. This will then appear as a drop down and you can select the required sequence. 
